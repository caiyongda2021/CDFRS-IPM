    ...
    while(l<5)
      {
        ...
        while(j<21)
        {
          while(i<5)
          {
            val higgs_CDFRS_sample_T1 = higgs_big_sample_RSP.map(k => k._2).map(v => v(nums(i).toInt)).coalesce(j.toInt, false, Option(new SonPartitionCoalescer(sampling_Without_Replacement(782, j.toInt).toArray)))
            val higgs_CDFRS_sample_T2 = higgs_big_sample_RSP.map(k => k._2).map(v=>v(nums(i).toInt)).coalesce((j+1).toInt, false, Option(new SonPartitionCoalescer(sampling_Without_Replacement(782, (j+1).toInt).toArray)))
