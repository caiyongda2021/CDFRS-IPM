...
object singleModelDT_higgs {
  def main(args: Array[String]): Unit = {
    ...
    val higgs_train_data_big_sample = higgs_train_data.coalesce(783, false, Option(new SonPartitionCoalescer(sampling_Without_Replacement(6000, 783).toArray)))
    ...
    while(i<2)
    {
      val higgs_train_data_CDFRS_sample = higgs_train_data_big_sample_RSP.coalesce(b, false, Option(new SonPartitionCoalescer(sampling_Without_Replacement(782, b).toArray)))
