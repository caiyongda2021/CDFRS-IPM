...
object kmeans_higgs {
  def main(args: Array[String]): Unit = {
    ...
    val higgs_big_sample = higgs_fulldataset.coalesce(783, false, Option(new SonPartitionCoalescer(sampling_Without_Replacement(8000, 783).toArray)))
    ...
    while(i<21)
      {
        val higgs_CDFRS_sample = higgs_big_sample_RSP.coalesce(b, false, Option(new SonPartitionCoalescer(sampling_Without_Replacement(782, b).toArray)))
    ...
    println("The total accuracy is:"+arr)
